...
- **自発的な会話**：キャラクターごとに有効化し、頻度と静かな時間帯を設定できます。
  誕生日に関連する会話にも対応します。
- **キャラクターの予定と知識**：週間予定、日付指定の予定、背景知識、人物関係を管理。
  チャットから予定を確認し、リマインダーや一緒の予定を作成できます。その日に会話があれば
  日記にもまとめられます。
- **会話履歴の改善**：上にスクロールすると古いメッセージを順次読み込みます。保存された
  音声はメッセージと紐付けられ、再起動後も再生できます。
- **動作・表情の設定**：感情カテゴリごとに固定またはランダムの演出をキャラクター別に
  設定できます。返信テキストの表示と同時に演出が始まります。
- **4 言語対応**：新機能も繁体字中国語、簡体字中国語、日本語、英語で利用できます。
...
| 動作と外観 | アクション、表情、視線、待機／ランダム動作、キャラクター別の感情演出、衣装、描画設定 |
| モデルとアセット | `dresses.json` フォルダーまたは単体 Spine 3.6 セットの取り込みと検証 |
| チャットと AI | キャラクター別チャット、自発的な会話、履歴の順次読み込み、OpenAI 互換 LLM、Persona、プロデューサー情報、画面コンテキスト |
| キャラクターの予定 | 週間予定、カレンダー予定、アラームとリマインダー、背景知識、人物関係、日記 |
