...
- **主動互動**：可逐一設定角色是否主動搭話、互動頻率與靜默時段，並支援生日相關互動。
- **角色行程與知識**：管理週作息、指定日期行程、背景知識與人物關係；聊天時可查詢行程、
  建立提醒與共同約會，並在有當日對話時整理日記。
- **更完整的聊天記錄**：向上瀏覽時分批載入舊訊息；語音與訊息建立索引，重啟後仍可重播
  已保存的語音。
- **動作與表情偏好**：依角色設定各情緒類別要固定播放或隨機選擇的動作、表情；回覆文字
  出現時同步觸發對應演出。
- **四語介面**：新增功能同步提供繁體中文、簡體中文、日文與英文介面。
...
| 動作與外觀 | 動作、表情、注視、待機／隨機動作、依角色設定情緒演出、服裝與渲染設定 |
| 模型與資產 | 匯入 `dresses.json` 資料夾或單一 Spine 3.6 資產組，檢查並集中保存 manifest |
| 聊天與 AI | 角色專屬聊天室、主動互動、分批載入歷史訊息、OpenAI 相容 LLM、Persona、製作人資料與畫面情境 |
| 角色行程 | 週作息表、月曆行程、鬧鐘與提醒、背景知識、人物關係與日記 |
