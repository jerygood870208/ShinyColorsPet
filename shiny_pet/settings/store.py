...
def validate(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in (
        ...
        "interaction_debug_hit_areas",
        "interaction_gaze_enabled",
        "interaction_idle_enabled",
        "interaction_random_enabled",
        "chat_debug_enabled",
    ):
    ...
    if (
        isinstance(asr_timeout, bool)
        or not isinstance(asr_timeout, (int, float))
        or not math.isfinite(asr_timeout)
        or not 5 <= asr_timeout <= 1200
    ):
    ...
    preferences = result["character_semantic_preferences"]
    if not isinstance(preferences, dict):
        raise ValueError("character_semantic_preferences must be an object")
    for character_id, kinds in preferences.items():
        if (
            not isinstance(character_id, str)
            or not character_id.strip()
            or not isinstance(kinds, dict)
        ):
            raise ValueError("invalid character semantic preference")
        for kind, categories in kinds.items():
            if kind not in {"gesture", "expression"} or not isinstance(categories, dict):
                raise ValueError("invalid semantic preference kind")
            if not all(
                isinstance(category, str) and category.strip()
                and isinstance(choice, str) and choice.strip()
                for category, choice in categories.items()
            ):
                raise ValueError("semantic preferences require non-empty text")
...
class SettingsStore:
    ...
    def load(self) -> dict[str, Any]:
        ...
        original_version = payload.get("settings_schema_version", 0)
        if original_version in (0, 2):
            backup_version = 1 if original_version == 0 else 2
            backup = self.path.with_name(self.path.name + f".pre-v{backup_version}.bak")
            ...
            # Persist retirement of obsolete settings even when the schema version itself
            # does not change, so removed integrations cannot reappear on the next launch.
            self.save(result)
        return result
